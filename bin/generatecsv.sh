...
if ($7 && $3 > 0) {NB = ($3 * 10000000)/ ($7 * 1); printf "%s;resultats/effectifs;%s;%.02f\n",BANQUE,$1,NB } ;
if ($3 > 0 && $2 > 0) {NB = $3 / $2; printf "%s;resultats/pnb;%s;%.02f\n",BANQUE,$1,NB } ;
...
if ($8 && $3 > 0) {NB = ($3 * 10000000)/ ($8 * 1); printf "%s;resultats/effectifs;%s;%.02f\n",BANQUE,$1,NB } ;
if ($3 > 0 && $2 > 0) {NB = $3 / $2; printf "%s;resultats/pnb;%s;%.02f\n",BANQUE,$1,NB } ;
}' ; done | sed 's/.csv//' | sed 's|data/export/||' | sed 's/_/;/' >> data/csv/indicateurs.csv
